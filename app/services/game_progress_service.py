...
def update_time_spent(email: str, game_id: str, time_spent: int):
    print(f"Updating time spent for {email} on game {game_id}: {time_spent} seconds")
    db.users.update_one(
        {"email": email},
        {"$inc": {f"games.{game_id}.timeSpent": time_spent/1000}} # making it seconds
    )